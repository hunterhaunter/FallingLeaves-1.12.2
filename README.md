...
Requires Forge 14.23.5.2860+. 100% client-side — no mixins, no coremod, and it joins any server.
...
**biome tint** the moment it spawns, so oak, birch, jungle, spruce and modded leaves all fall in
...
you'd rather not see both. Turn the notice off with `showCompatWarning` in the config. Colour mods
like Serene Seasons are deliberately **not** flagged — they recolour leaves rather than adding
particles, so they're fully compatible.
